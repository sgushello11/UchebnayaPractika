...
namespace HranitelProGeneralDept
{
    public class DatabaseHelper
    {
        ...
        // 1.1. Отчет по количеству посещений
        public DataTable GetVisitsReport(string period, string department = null)
        {
            string sql = "";

            if (period == "день")
            {
                sql = @"SELECT TO_CHAR(createdat, 'DD.MM.YYYY') as Дата, 
                       targetdepartment as Подразделение, 
                       COUNT(*) as Количество
                FROM visitrequests 
                WHERE status = 'одобрена'";

                if (!string.IsNullOrEmpty(department) && department != "Все")
                    sql += $" AND targetdepartment = '{department}'";

                sql += " GROUP BY TO_CHAR(createdat, 'DD.MM.YYYY'), targetdepartment ORDER BY Дата DESC";
            }
            else if (period == "месяц")
            {
                sql = @"SELECT TO_CHAR(createdat, 'MM.YYYY') as Месяц, 
                       targetdepartment as Подразделение, 
                       COUNT(*) as Количество
                FROM visitrequests 
                WHERE status = 'одобрена'";
                ...
                sql += " GROUP BY TO_CHAR(createdat, 'MM.YYYY'), targetdepartment ORDER BY Месяц DESC";
            }
            else if (period == "год")
            {
                sql = @"SELECT TO_CHAR(createdat, 'YYYY') as Год, 
                       targetdepartment as Подразделение, 
                       COUNT(*) as Количество
                FROM visitrequests 
                WHERE status = 'одобрена'";
                ...
                sql += " GROUP BY TO_CHAR(createdat, 'YYYY'), targetdepartment ORDER BY Год DESC";
            }
        ...
        // 1.2. Список лиц на территории
        public DataTable GetCurrentVisitors()
        {
            string sql = @"SELECT v.visitor_lastname as Фамилия, v.visitor_firstname as Имя, 
                          v.visitor_patronymic as Отчество, v.targetdepartment as Подразделение,
                          v.actual_entry_time as Время_входа, v.visitpurpose as Цель
                   FROM visitrequests v
                   WHERE v.status = 'одобрена' 
                     AND v.actual_entry_time IS NOT NULL 
                     AND v.actual_exit_time IS NULL
                   ORDER BY v.targetdepartment, v.visitor_lastname";

            return Query(sql);
        }

        // Отчет по количеству посетителей за период (3 часа)
        public DataTable GetVisitorsByDepartmentReport(DateTime startTime, DateTime endTime)
        {
            DataTable result = new DataTable();
            result.Columns.Add("Подразделение");
            result.Columns.Add("Количество");
            result.Columns.Add("Первое_посещение");
            result.Columns.Add("Последнее_посещение");
            ...
            string sql = @"SELECT 
                   ...
                   GROUP BY targetdepartment
                   ORDER BY targetdepartment";

            var dt = Query(sql,
                new NpgsqlParameter("start", startTime),
                new NpgsqlParameter("end", endTime));

            // Если данных нет, добавляем строку с сообщением
            if (dt.Rows.Count == 0)
            {
                result.Rows.Add("Нет данных за указанный период", 0, "", "");
                return result;
            }
