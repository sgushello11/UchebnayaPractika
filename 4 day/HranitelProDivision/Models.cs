using System;
using System.Collections.Generic;

namespace HranitelProDivision
{
    public class User
    {
        public int EmployeeID { get; set; }
        public string LastName { get; set; } = "";
        public string FirstName { get; set; } = "";
        public string Patronymic { get; set; } = "";
        public string EmployeeCode { get; set; } = "";
        public string Department { get; set; } = "";
        public string FullName => $"{LastName} {FirstName} {Patronymic}".Trim();
    }

    public class VisitRequest
    {
        public int RequestID { get; set; }
        public string RequestType { get; set; } = "";
        public string Status { get; set; } = "";
        public DateTime StartDate { get; set; }
        public DateTime EndDate { get; set; }
        public string VisitPurpose { get; set; } = "";
        public string TargetDepartment { get; set; } = "";
        public string Note { get; set; } = "";

        // Данные посетителя
        public string VisitorLastName { get; set; } = "";
        public string VisitorFirstName { get; set; } = "";
        public string VisitorPatronymic { get; set; } = "";
        public string VisitorPhone { get; set; } = "";
        public string VisitorEmail { get; set; } = "";
        public string VisitorOrganization { get; set; } = "";
        public DateTime VisitorBirthDate { get; set; }
        public string VisitorPassportData { get; set; } = "";

        // Время выхода
        public DateTime? ActualEntryTime { get; set; }      // от охраны (разрешение доступа)
        public DateTime? DivisionEntryTime { get; set; }    // от подразделения (фактический приход)
        public DateTime? ActualExitTime { get; set; }       // выход

        public string FullName => $"{VisitorLastName} {VisitorFirstName} {VisitorPatronymic}".Trim();
    }

    public class BlacklistItem
    {
        public int Id { get; set; }
        public string LastName { get; set; } = "";
        public string FirstName { get; set; } = "";
        public string Patronymic { get; set; } = "";
        public string PassportData { get; set; } = "";
        public string Reason { get; set; } = "";
        public DateTime AddedAt { get; set; }
        public string FullName => $"{LastName} {FirstName} {Patronymic}".Trim();
    }
}